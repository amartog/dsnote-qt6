...
QString path_to_dir_for_path(const QString& dir, const QString& path) {
    ...
    auto path_full = QString("INSTALL_PREFIX") + QString("/share/%1/%2/%3")
                         .arg(APP_BINARY_ID, dir, path);
    if (QFileInfo::exists(path_full))
        return QString("INSTALL_PREFIX") + QString("/share/%1/%2")
            .arg(APP_BINARY_ID, dir);

    path_full = QString("INSTALL_PREFIX") + QString("/%1/%2").arg(dir, path);
    if (QFileInfo::exists(path_full))
        return QString("INSTALL_PREFIX") + QString("/%1").arg(dir);

    path_full = QString("INSTALL_PREFIX") + QString("/share/%1/%2").arg(APP_BINARY_ID, path);
    if (QFileInfo::exists(path_full))
        return QString("INSTALL_PREFIX") + QString("/share/%1").arg(APP_BINARY_ID);
