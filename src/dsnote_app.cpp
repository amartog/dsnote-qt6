...
void dsnote_app::create_player() {
    ...
    // m_player->setNotifyInterval(100); // Removed in Qt6?
    ...
    switch (rule.type) {
        ...
        case trans_rule_type_t::TransRuleTypeMatchRe: {
            QRegularExpression rx{
                rule.pattern,
                case_sens ? QRegularExpression::NoPatternOption
...
void dsnote_app::play_speech_selected(int start, int end) {
    ...
    start = std::clamp(start, 0, static_cast<int>(size));
    end = end < 0 ? static_cast<int>(size) : std::clamp(end, start, static_cast<int>(size));
...
QString dsnote_app::tts_ref_voice_unique_name(QString name,
    ...
    if (auto match = rx.match(name); match.hasMatch()) {
        bool ok = false;
        auto ii = name.mid(match.capturedStart(), match.capturedLength()).toInt(&ok);
        if (ok && ii < 99999) {
            i = ii;
            name = name.left(match.capturedStart()) + "%1";
        } else {
            name += " %1";
        }
