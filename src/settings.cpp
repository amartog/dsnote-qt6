...
static QString file_save_filename(const QDir& dir, QString filename,
    ...
    if (filename.isEmpty()) {
        ...
        if (auto match = rx.match(filename); match.hasMatch()) {
            bool ok = false;
            auto ii = filename.mid(match.capturedStart(), match.capturedLength()).toInt(&ok);
            if (ok && ii < max_i) {
                i = ii;
                filename = filename.left(match.capturedStart()) + "%1." + ext;
            } else {
                filename += "-%1." + ext;
            }
